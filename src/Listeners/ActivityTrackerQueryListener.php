 ...
 *
 * Plain SELECT queries — and, per the same reasoning, count()/exists() —
 * are intentionally never logged here. Eloquent's retrieved/created/updated
 * events already cover model-level read/write semantics for SELECTs, and
 * count()/exists() results carry essentially no audit value on their own
 * (see README § Tracked operations for the removal rationale).
 *
 ...
 * count. This listener can reliably say *that* a sum/avg/min/max/bulk
 ...
 */
final class ActivityTrackerQueryListener
{
    /**
     * Classifications that are never turned into an activity, regardless of
     * table or config — count()/exists() have no audit value, and plain
     * selects are already represented by retrieved/retrieved_many.
     */
    private const NEVER_TRACKED = ['select', 'unknown', 'count', 'exists'];
    ...
    public function handle(QueryExecuted $event): void
    {
        ...
        if (in_array($classification, self::NEVER_TRACKED, true)) {
            return;
        }
        ...
        $this->tracker->logQueryEvent($classification, [
            ...
            'duration_ms' => $this->durationFor($event),
    ...
    /**
     * Laravel's QueryExecuted::$time is already a high-precision millisecond
     * duration for exactly this SQL statement — no need to hand-roll a
     * timer here, unlike the Eloquent-hook-driven activities.
     */
    private function durationFor(QueryExecuted $event): ?float
    {
        if (! config('activity-tracker.performance.enabled', true) || ! config('activity-tracker.performance.track_duration', true)) {
            return null;
        }
