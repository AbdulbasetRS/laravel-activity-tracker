 ...
 * own activity.
 */
final class ActivityTrackerObserver
{
    /**
    ...
    private function startTimingAndExpectQuery(Model $model, string $queryType): void
    {
        if ($this->durationTrackingEnabled()) {
            $this->trackingContext->startTimer(spl_object_id($model));
        }
    ...
    {
        $this->tracker->logModelEvent('created', $model, array_merge([
            'new_values' => $this->transformer->attributesFor($model),
        ], $this->performanceExtra($model)));
    }
    ...
    private function handleUpdated(Model $model): void
    {
        ...
        $performance = $this->performanceExtra($model);
        ...
        if (isset($this->restoringModels[$id])) {
            ...
            if ($diff['changed_values'] !== []) {
                $this->pendingRestoreDiffs[$id] = array_merge($diff, $performance);
            }
        ...
        $this->tracker->logModelEvent('updated', $model, array_merge($diff, $performance));
    }
    ...
    private function handleDeleted(Model $model): void
    {
        ...
        $this->tracker->logModelEvent('deleted', $model, array_merge([
    ...
    private function handleRestored(Model $model): void
    {
        ...
        $extra = $this->pendingRestoreDiffs[$id] ?? $this->performanceExtra($model);
    ...
    {
        $id = spl_object_id($model);
        unset($this->forceDeletingModels[$id]);

        $this->tracker->logModelEvent('force_deleted', $model, array_merge([
            'old_values' => $this->transformer->attributesFor($model),
        ], $this->performanceExtra($model)));
    }

    /**
     * Stops the timer started on the matching "-ing" pre-hook and returns
     * duration_ms (null if, for whatever reason, no timer was running —
     * this must never throw or block logging). Also snapshots current
     * memory usage when performance.track_memory is enabled.
     *
     * @return array<string, mixed>
     */
    private function performanceExtra(Model $model): array
    {
        $duration = $this->durationTrackingEnabled()
            ? $this->trackingContext->stopTimer(spl_object_id($model))
            : null;

        $extra = ['duration_ms' => $duration];

        if (config('activity-tracker.performance.track_memory', false)) {
    ...
    private function durationTrackingEnabled(): bool
    {
        return config('activity-tracker.performance.enabled', true)
            && config('activity-tracker.performance.track_duration', true);
    }
